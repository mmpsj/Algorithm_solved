...
처음엔 다리가 겹치면 안된다는 말에 집중해서 경우를 어떻게 나눠야 하지,,, 하면서 고민을 잠깐 했다. 근데 떠오르는 아이디어가 없어서 다른 접근이 필요했다.

우선 다리의 개수는 n개이고, 이 n개의 다리를 어떻게 배치할지가 문제인 것이다. 

다리를 겹치지 않게 한다는 것은 다리의 순서가 정해져 있다는 것이다. 그렇다면 오른쪽의 m개 중에서 n개가 다리에 연결되게만 하면 되는 것이다. 즉, mCn을 구하면 되는 것이다.

[11050번 문제](https://github.com/mmpsj/Baekjoon/tree/main/%EB%B0%B1%EC%A4%80/Bronze/11050.%E2%80%85%EC%9D%B4%ED%95%AD%E2%80%85%EA%B3%84%EC%88%98%E2%80%851)의 방법으로 해당 값을 계산해서 결과로 출력하면 된다.
