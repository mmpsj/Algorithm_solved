...
"ENTER"가 나오기 전까지 입력된 문자열(멤버)들을 저장하고, "ENTER"가 나오면 저장된 개수를 총 개수에 더한다. 이때 저장되는 자료형은 중복을 허용하지 않는 자료형인 파이썬의 set을 사용했다. 또는 dictionary를 이용해도 괜찮을 것 같다.

이와 별개로 입력을 input()으로 받았을 때는 2초가 넘게 걸렸는데, 찾아보니 sys.stdin.readline()을 사용하니 시간이 68ms로 훨씬 줄어들었다. 이 함수를 사용할 때는 rstrip()으로 공백을 제거해 주어야 한다.
