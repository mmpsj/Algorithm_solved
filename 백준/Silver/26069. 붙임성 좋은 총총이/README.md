...
처음 총총이가 나오고 나서 만나는 사람들만 춤을 추게 되므로, 총총이가 나오기 전까지 만나는 사람들은 무시해도 된다.

총총이가 나오고 나서는 만나는 모든 사람을 저장한 후 그 개수를 확인하면 되는데, 한 사람이 여러 번 만날 수 있으므로 파이썬의 set을 이용해 중복을 허용하지 않도록 한다. 이후 이 set의 크기를 출력하면 된다.
