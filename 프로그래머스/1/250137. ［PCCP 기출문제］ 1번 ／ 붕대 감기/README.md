...
회복에 성공하는 횟수는 다음 공격 시간 - 마지막으로 공격받은 시간 - 1이다. 해당 횟수를 시전 시간으로 나눠 나온 몫은 추가 회복 횟수이다. 두 횟수를 저장한 후, 각각의 회복량과 곱해 현재 체력에 더하면 마지막으로 받은 공격과 다음 공격 사이의 회복량이 계산된다. 이때 최대 체력과 비교하여 더 낮은 값을 현재 체력으로 정한다. 회복을 마치면, 다음 공격을 정산하면 된다. 현재 체력을 공격력만큼 감소시키고, 마지막으로 공격받은 시간을 업데이트한다. 만약 현재 체력이 0 이하가 되면 -1을 반환한다. 0 초과인 채로 끝났다면 체력을 반환한다.
